...
class VendorDetailView extends GetView<VendorController> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          CustomScrollView(
            slivers: [
              // Image carousel
              SliverToBoxAdapter(child: _buildCarousel(vendor, pageController, currentPage)),
              // Basic info
              SliverToBoxAdapter(child: _buildBasicInfo(vendor)),
              // Description
              SliverToBoxAdapter(child: _buildSection('Tentang Vendor',
                  Padding(padding: const EdgeInsets.symmetric(horizontal: 20),
                    child: Text(vendor.description, style: GoogleFonts.poppins(fontSize: 13, color: AppColors.textSecondary, height: 1.7))))),
              // Packages
              SliverToBoxAdapter(child: _buildSection('Paket Layanan', _buildPackages(vendor))),
              // Reviews
              SliverToBoxAdapter(child: _buildSection('Ulasan', _buildReviews(vendor))),
      ...
      bottomNavigationBar: _buildBottomBar(vendor),
    );
  }
  ...
  Widget _buildCarousel(VendorModel vendor, PageController pc, RxInt page) {
    final imgs = [vendor.imageUrl, ...vendor.gallery];
    return SizedBox(
      height: 280,
      child: Stack(
        children: [
  ...
  Widget _buildBasicInfo(VendorModel vendor) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(vendor.name, style: GoogleFonts.poppins(fontSize: 20, fontWeight: FontWeight.w700)),
            ...
            child: GradientButton(
              ...
              onTap: () {
                if (vendor.packages.isNotEmpty) {
                  controller.goToBooking(vendor.packages.first);
                }
              },
            ),
          ),
